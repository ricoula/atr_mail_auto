        ...
        try{
            ...
            if($liste != null)
            {
                ...
                foreach($listePoi as $poi)
                {
                    ...
                    if($idPoi != null)
                    {
                        $req = $bdd->prepare("UPDATE relance SET nb_relances = nb_relances + 1, date_derniere_relance = NOW() WHERE id = ? RETURNING nb_relances");
                        $req->execute(array($idPoi));
                        if($data = $req->fetch())
                        {
                            $reponse = true;
                            $listePoiRelance["poi-".$poi] = $data["nb_relances"];
                        }
                    }
                }
                    ...
                    if($liste != null)
                    {
                        foreach($liste as $caff)
                        {
                            ...
                            if($caff->listePois != null)
                            {
                                ...
                                {
                                    $nbRelancesPoi = 0;
                                    foreach($listePoiRelance as $key=>$val)
                                    {
                                        if($key == "poi-".$poi->id)
                                        {
                                            $poi->nbRelances = $val;
                                            if($poi->nbRelances >= $limiteNbRelances )
                                            {
                                                $limiteRelance = true;
                                            }
                                        }
                                    }
                                    if(!isset($poi->nbRelances) || $poi->nbRelances == null)
                                    {
                                        $poi->nbRelances = 0;
                                    }
                                    $contenuHtml = $contenuHtml."<tr><td>".$poi->atr_ui ."</td><td>".$poi->ft_numero_oeie ."</td><td>".$poi->ft_oeie_dre ."</td><td>".$poi->domaine ."</td><td>".$poi->sous_domaine ."</td><td>".$poi->ft_pg ."</td><td>".$poi->ft_sous_justification_oeie ."</td><td>".$poi->ft_libelle_commune ."</td><td>".$poi->ft_libelle_de_voie ."</td><td>".$poi->ft_commentaire_creation_oeie ."</td><td>".$poi->nbRelances."</td></tr>";
                                }
                            }
                            ...
                            if($limiteRelance)
                            {
                                $req = $bddErp->prepare("SELECT work_email FROM hr_employee WHERE id = (SELECT parent_id FROM hr_employee WHERE name_related = ?)");
                                $req->execute(array($caff->nom));
                                if($data = $req->fetch())
                                {
